...
- [新增] ComfyUI 工作流节点可在参数面板重新选择已暴露的输入与输出，并为图片、视频和音频输入提供“使用 / 跳过”开关。
- [修复] ComfyUI 工作流将 VHS Video Combine 的 `VHS_FILENAMES` 输出识别为视频，生成结果直接进入视频节点，不再显示原始 `gifs` JSON。
- [修复] Windows 桌面客户端允许连接本机回环地址上的 TDCanvas Agent，避免正式构建被 CSP 拦截后持续显示连接失败。
- [新增] README 与 ComfyUI 本地模块文档增加土豆纯净整合包下载入口及画布插件化接入步骤，注明无模型、包含启动器。
- [新增] README 展示区补充桌面项目管理与多素材创作全景截图。
- [新增] README 增加深浅主题画布与 API 接入页的产品截图。

- [调整] 移除旧项目存储兼容层，画布状态与素材统一使用 TDCanvas 本地存储。

- [调整] README 来源说明精简为一句，贡献协议项目名称及中英文文档链接统一为 TDCanvas。
- [优化] API 设置改为醒目的整行注册按钮与两步接入引导，README 顶部新增 AI 土豆 API 注册、画布连接和二次开发入口。
...
- [优化] API 配置界面新增 AI 土豆注册与 API_KEY 获取引导，可直接访问官网完成注册和密钥创建。
...
- [优化] Windows 主窗口改为与画布融合的 TDCanvas 自定义标题栏，支持拖动、双击最大化、主题同步及最小化/还原/关闭；macOS 继续保留原生标题栏。
- [调整] 正式用户入口迁移为 Tauri 2 桌面客户端，新增 Windows/macOS 原生窗口、独立启动动画、原生 Aitudou 网络访问、系统应用目录媒体缓存和原生文件保存。
- [新增] 新增可交互选择 Windows、macOS 或全部目标的桌面打包脚本及双平台 GitHub Actions；Windows 输出 NSIS 安装程序，macOS 输出通用 `.app/.dmg`。
- [调整] 根路径改为直接打开最近使用的 TDCanvas 画布；移除展示首页、独立生图工作台、独立视频创作台及其 Agent 工具入口。
- [调整] 清空预置模型渠道、模型、提示词来源、WebDAV 目录及官方文档、插件注册表和版本检查地址，后续均由用户或环境变量显式配置。
- [新增] TDCanvas 画布背景新增低干扰的流动环境光、呼吸网格与指针响应，并支持系统减少动态效果设置。
- [调整] 将产品展示名、内部标识、存储命名、Agent、MCP 与本地插件命名统一为 TDCanvas / TDTV / `tdcanvas`。
- [新增] 画布新增 Aitudou 专用任务节点，按官方文档对接通用生成、钱包、上传、转写、16 项 Midjourney 与 31 项 Suno 能力；结果展开为图片、视频、音频、文本或文件节点，并立即转存到本地持久存储。
- [新增] 收录并校验 Aitudou 官方 123 个媒体模型及 Kimi、Whisper、Suno 模型，按能力、输入类型和已确认限制提供选择与提交前校验。
- [新增] Aitudou 异步任务支持多 Task ID、部分成功、进度、交互等待态、停止及恢复本地轮询；提交时同步写入恢复日志，本地开发与预览通过同源代理上传画布素材，并明确停止轮询不会取消远端任务。
...
- [优化] 图片节点按 RHTV 的基础操作顺序收敛为模型、比例/分辨率、生成数量、价格与生成；比例只允许模型预设，Aitudou GPT 图片补齐比例选择，Midjourney 仅保留比例、生成速度与 HD，并移除“更多操作”和高级参数入口；不支持数量字段的模型会把 1–4x 安全拆分为独立单结果任务。
- [修复] Aitudou Image G-2 固定使用官方接口开放的 1k，并清理 RHTV 直连模型才具备的图像质量、2k、4k 等未文档化字段，避免错误请求。
- [修复] 画布图片、视频、音频与文件节点下载优先读取本地持久化 Blob；浏览器无法跨域读取的公开 HTTPS 媒体会自动通过受控同源中转保存，不再打开新标签或要求重新生成素材。
- [修复] Aitudou 图片与视频结果按真实媒体比例调整节点且保持中心，图片和视频统一填满预览容器，避免默认 16:9 节点造成裁切、透明空带或自由拉伸变形。
- [新增] 图片、视频、音频与文本节点的模型列表显示官方人民币参考价或结算规则，并在生成按钮旁按当前模型、参数与数量展示预计费用；动态价格优先读取 Aitudou 官方价格清单。
...
- [新增] Aitudou 图片、视频、音频与文件结果完成后优先自动下载到仓库 `data/media-cache`，画布保存稳定的同源地址和可见磁盘路径，并继续保留浏览器 Blob 副本；历史临时地址在裁剪或下载时也会尝试补写磁盘缓存。
