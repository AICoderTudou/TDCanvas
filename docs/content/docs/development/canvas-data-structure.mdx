...
Canvas projects and My Assets are stored with localForage in the `tdcanvas` database and `app_state` store. The persistent keys are `tdcanvas:canvas_store` and `tdcanvas:asset_store`. Image blobs use `image_files`; other media blobs use `media_files`. Project JSON stores display URLs, `storageKey`, and metadata rather than large base64 payloads.
