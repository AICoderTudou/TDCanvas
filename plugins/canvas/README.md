...
作者只写节点 UI 与逻辑,**类型、JSX、宿主 React、构建全部由 [`@tdtv/plugin-sdk`](./sdk/README.md) 提供**,写 TSX 全程有代码提示;产物仍是宿主加载器现有契约的 ESM(React external、宿主单例)。
...
```tsx
import { definePlugin, useState } from "@tdtv/plugin-sdk";
import type { CanvasNodeContentProps } from "@tdtv/plugin-sdk";
...
```ts
{
    ...
    ports?: CanvasNodePort[] | ((node) => CanvasNodePort[]); // 可选:具名、带类型的动态输入/输出端口
    keepAspectRatio?: (node) => boolean;
    resource?: (node, outputPortId?) => { kind: "text"|"image"|"video"|"audio", text?, url? } | null; // 按输出端口提供资源
...
| `ctx.getInputConnections(portId?)` / `ctx.getOutputConnections(portId?)` | 按端口读取连接 |
| `ctx.getUpstream(portId?)` / `ctx.getDownstream(portId?)` | 按端口取上/下游相连节点 |
...
ctx.applyOps([
    ...
    { type: "connect_nodes", fromNodeId, toNodeId, fromPortId?, toPortId? },
...
没有声明 `ports` 的节点继续使用旧版左右单连接点。工作流类节点可以声明动态端口；端口 `id` 必须稳定且不可使用显示顺序，`dataType` 相同（或任一端为 `any`）才能连接。输入端口默认只接受一条入线，设置 `multiple: true` 后可接受多条。多输出节点必须在 `resource(node, outputPortId)` 中按端口返回对应资源。
