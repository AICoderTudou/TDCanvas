...
export type CanvasBuiltinNodeType =
  | "image"
  | "text"
  | "config"
  | "video"
...
export type CanvasNodePort = {
  ...
  required?: boolean;
  multiple?: boolean; // 输入端口默认只允许一条入线;true 表示允许多条。
  description?: string;
  color?: string;
};
...
export type CanvasNodeDefinition = {
  ...
  createMenuPlacement?: "primary" | "extension"; // 可选择与原生媒体节点同级展示
  hasSourceHandle?: boolean; // 右侧输出连接点,默认 true
  // 具名、带类型的动态端口;不声明时保留旧版左右单连接点。
