...
import { CanvasResizableArea } from "./canvas-resizable-area";
import { CanvasObjectReferencePicker } from "./canvas-object-reference-picker";
...
export function CanvasNodePromptPanel({ node, isRunning, onPromptChange, onConfigChange, onGenerate, onStop, mentionReferences = [], canvasNodes = [], objectMode = false, onFocusReference, onReorderReferences, onRemoveReference, onReorderReference, onAddObjectReference, onRemoveObjectReference, onImageSettingsOpenChange, modeOverride }: CanvasNodePromptPanelProps) {
        ...
        >
            {mentionReferences.some((reference) => reference.active && reference.nodeId !== node.id) ? (
                <CanvasResizableArea storageKey={`tdcanvas:node-reference-height:${node.id}`} defaultHeight={84} minHeight={72} maxHeight={220} resizeLabel={t("canvas.references.resizePreview")} className="mb-2.5">
                    <CanvasConnectedReferences
                        ...
                        onReorder={isRunning ? undefined : onReorderReferences}
                        onRemoveReference={isRunning ? undefined : onRemoveReference}
                        onReorderReference={isRunning ? undefined : onReorderReference}
                    />
                </CanvasResizableArea>
            ) : null}
            ...
            <CanvasResizableArea storageKey={`tdcanvas:node-prompt-height:${node.id}`} defaultHeight={160} minHeight={96} maxHeight={420} resizeLabel={t("canvas.promptPanel.resizeInput")}>
                <CanvasPromptChipInput
                    ...
                    onSubmit={submit}
                    className="thin-scrollbar h-full w-full cursor-text rounded-xl px-3 py-2 text-sm leading-5 outline-none"
