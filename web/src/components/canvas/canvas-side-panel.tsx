...
const PANEL_MOTION_SECONDS = Math.min(CANVAS_SIDE_PANEL_MOTION_MS / 1000, 0.24);
...
export function CanvasSidePanel({ nodes, selectedNodeIds, onFocusNode, onPreviewNode, onInsertAsset }: Props) {
    ...
    const setTab = useCanvasSidePanelStore((state) => state.setActiveTab);
    ...
    return (
        <motion.div
            className="pointer-events-none absolute bottom-3 left-14 top-14 z-[60] flex"
            ...
            style={{ overflow: "clip", pointerEvents: panelClosing || !panelOpen ? "none" : "auto", maxWidth: "calc(100vw - 68px)" }}
        >
            <motion.aside
                className="td-canvas-side-panel relative flex h-full shrink-0 flex-col overflow-hidden rounded-2xl border backdrop-blur-xl"
                initial={{ x: -18 }}
                animate={{ x: panelClosing ? -12 : 0 }}
                transition={{ duration: resizing ? 0 : PANEL_MOTION_SECONDS, ease: PANEL_EASE }}
                style={{
                    width,
                    maxWidth: "calc(100vw - 68px)",
                    background: colorTheme === "dark" ? "rgba(16, 18, 19, .94)" : "rgba(252, 252, 250, .95)",
                    borderColor: colorTheme === "dark" ? "rgba(255,255,255,.09)" : "rgba(28,25,23,.11)",
                    color: theme.node.text,
                    boxShadow: colorTheme === "dark" ? "0 22px 56px rgba(0,0,0,.32)" : "0 20px 48px rgba(28,25,23,.12)",
                }}
                data-canvas-no-zoom
            >
