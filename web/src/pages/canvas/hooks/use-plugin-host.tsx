...
export function usePluginHost(params: PluginHostParams) {
    ...
    const pluginHost = useMemo<CanvasPluginHost>(
        () => ({
            ...
            getInputConnections: (nodeId, portId) => connectionsRef.current.filter((connection) => connection.toNodeId === nodeId && (portId === undefined || connection.toPortId === portId)),
            getOutputConnections: (nodeId, portId) => connectionsRef.current.filter((connection) => connection.fromNodeId === nodeId && (portId === undefined || connection.fromPortId === portId)),
            getUpstream: (nodeId, portId) =>
                connectionsRef.current
                    .filter((conn) => conn.toNodeId === nodeId && (portId === undefined || conn.toPortId === portId))
                    .map((conn) => nodesRef.current.find((node) => node.id === conn.fromNodeId))
                    .filter((node): node is CanvasNodeData => Boolean(node)),
            getDownstream: (nodeId, portId) =>
                connectionsRef.current
                    .filter((conn) => conn.fromNodeId === nodeId && (portId === undefined || conn.fromPortId === portId))
