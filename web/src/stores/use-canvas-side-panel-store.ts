...
export const CANVAS_SIDE_PANEL_MOTION_MS = 220;
...
type CanvasSidePanelStore = {
    ...
    setActiveTab: (tab: CanvasSidePanelTab) => void;
    openPanel: (tab?: CanvasSidePanelTab) => void;
    closePanel: () => void;
    togglePanel: () => void;
};
...
export const useCanvasSidePanelStore = create<CanvasSidePanelStore>((set, get) => ({
    ...
    openPanel: (activeTab) => {
        ...
        set({ panelOpen: true, panelMounted: true, panelClosing: false, ...(activeTab ? { activeTab } : {}) });
    },
    closePanel: () => {
