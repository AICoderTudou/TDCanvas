...
export const defaultConfig: AiConfig = {
    ...
    apiKey: "",
    apiFormat: "aitudou",
    channels: [createOfficialAitudouChannel()],
    model: "",
    imageModel: "",
    videoModel: "",
    textModel: "",
...
export function selectableModelsByCapability(config: AiConfig, capability?: ModelCapability) {
    if (!capability) return config.channels.flatMap((channel) => channel.models.map((model) => encodeChannelModel(channel.id, model.name)));
...
function isAiConfigReady(config: AiConfig, model: string) {
    ...
    if (channel.apiFormat === "aitudou") return Boolean(channel.apiKey.trim());
    return Boolean(model.trim() && channel.apiKey.trim() && channel.baseUrl.trim());
}

function createAitudouOnlyConfig(apiKey: string): AiConfig {
    const normalizedApiKey = apiKey.trim();
    return {
        ...defaultConfig,
        apiKey: normalizedApiKey,
        channels: [createOfficialAitudouChannel(normalizedApiKey)],
    };
}
    ...
    persist(
        (set, get) => ({
            ...
            updateConfig: (key, value) =>
                set((state) => {
                    if (key === "apiKey") return { config: createAitudouOnlyConfig(String(value)) };
                    if (key === "channels") {
                        const channels = value as ModelChannel[];
                        const channelApiKey = channels.find((channel) => channel.apiFormat === "aitudou")?.apiKey;
                        return channelApiKey === undefined ? { config: state.config } : { config: createAitudouOnlyConfig(channelApiKey) };
                    }
                    if (key === "baseUrl" || key === "apiFormat" || key === "channelMode" || key === "models") return { config: state.config };
        ...
        }),
        {
            ...
            partialize: (state) => ({ config: createAitudouOnlyConfig(state.config.apiKey) }),
            merge: (persisted, current) => {
                ...
                return {
                    ...current,
                    webdav: defaultWebdavSyncConfig,
                    config: createAitudouOnlyConfig(persistedAitudouApiKey(persistedConfig)),
                };
            },
        },
    ),
...
export function resolveModelRequestConfig(config: AiConfig, value: string) {
    ...
    return {
        ...
        baseUrl: channel.apiFormat === "aitudou" ? channel.baseUrl.trim() || AITUDOU_OFFICIAL_BASE_URL : channel.baseUrl,
        apiKey: channel.apiKey,
...
function normalizeApiFormat(apiFormat: unknown): ApiCallFormat {
    return apiFormat === "gemini" || apiFormat === "ark" || apiFormat === "aitudou" ? apiFormat : "openai";
}
